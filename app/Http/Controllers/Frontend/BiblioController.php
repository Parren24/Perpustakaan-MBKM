...
class BiblioController extends Controller
{
    ...
    /**
     * Add book to cart (Mobile)
     */
    public function addBookToCartLoan(Request $request)
    {
        try {
            $content = SafeDataService::safeExecute(
                fn() => BiblioService::addBookToCartLoan($request)
            );
            return $content;
        } catch (\Exception $e) {
            Log::error('BiblioController addBookToCartLoan error: ' . $e->getMessage());
            return response()->json([
                'success' => false,
    ...
    public function getCartItems()
    {
        try {
            $content = SafeDataService::safeExecute(
                fn() => BiblioService::getCartItems()
            );
            return $content;
        } catch (\Exception $e) {
            Log::error('BiblioController getCartItems error: ' . $e->getMessage());
            return response()->json([
                'success' => false,
                'error' => 'Terjadi kesalahan sistem. Silakan coba lagi.'
            ], 500);
    ...
    public function removeFromCart(Request $request)
    {
        try {
            $content = SafeDataService::safeExecute(
                fn() => BiblioService::removeFromCart($request)
            );
            return $content;
        } catch (\Exception $e) {
            Log::error('BiblioController removeFromCart error: ' . $e->getMessage());
            return response()->json([
                'success' => false,
                'error' => 'Terjadi kesalahan sistem. Silakan coba lagi.'
            ], 500);
    ...
    /**
     * Clear all items from cart (Mobile)
     */
    public function clearCart()
    {
        try {
            $content = SafeDataService::safeExecute(
                fn() => BiblioService::clearCart()
            );
            return $content;
        } catch (\Exception $e) {
            Log::error('BiblioController clearCart error: ' . $e->getMessage());
            return response()->json([
                'success' => false,
                'error' => 'Terjadi kesalahan sistem. Silakan coba lagi.'
            ], 500);
    ...
    /**
     * Complete loan transaction (Mobile)
     */
    public function completeLoan(Request $request)
    {
        try {
            $content = SafeDataService::safeExecute(
                fn() => BiblioService::storeLoanTransaction($request)
            );
            return $content;
        } catch (\Exception $e) {
